...
class EditingTextStateTest {
    ...
    @Test
    void testHandleKeyTypedAddsCharacter() {
        KeyEvent keyEvent = mock(KeyEvent.class);
        when(keyEvent.getCharacter()).thenReturn("a");
        when(shape.getText()).thenReturn("Hello");

        editingState.handleKeyTyped(keyEvent, Collections.singletonMap(shape, view));

        verify(shape).setText("Helloa");
        verify(keyEvent).consume();
        verify(StateController.getInstance()).redrawCanvas();
    }

    @Test
    void testHandleKeyTypedBackspace() {
        KeyEvent keyEvent = mock(KeyEvent.class);
        when(keyEvent.getCharacter()).thenReturn("\b");
        when(shape.getText()).thenReturn("Hello");

        editingState.handleKeyTyped(keyEvent, Collections.singletonMap(shape, view));

        verify(shape).setText("Hell");
        verify(keyEvent).consume();
        verify(StateController.getInstance()).redrawCanvas();
    }

    @Test
    void testHandleKeyTypedOnEmptyText() {
        when(shape.getText()).thenReturn("");
        KeyEvent keyEvent = mock(KeyEvent.class);
        when(keyEvent.getCharacter()).thenReturn("X");

        editingState.handleKeyTyped(keyEvent, Collections.singletonMap(shape, view));

        verify(shape).setText("X");
        verify(StateController.getInstance()).redrawCanvas();
    }
}
