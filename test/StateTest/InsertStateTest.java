...
class InsertStateTest {
    ...
    @Test
    void testHandleClickCreatesShapeAndExecutesCommand() {
        MouseEvent event = mock(MouseEvent.class);
        Map<Shape, ShapeView> map = new HashMap<>();

        state.handleClick(event, 100.0, 200.0, map);

        // Verifica che il creator venga chiamato
        verify(creator).createShape(eq(100.0), eq(200.0), eq(100.0), eq(50.0),
                any(), any(), eq(1), eq(0));
        verify(creator).createShapeView(shape);

        // Verifica che venga eseguito un comando di inserimento
