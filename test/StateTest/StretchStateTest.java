...
class StretchStateTest {
    ...
    @Test
    void testHandleMouseDraggedTriggersStretchDown() {
        stretchState.startDragging(100, 120); // parte sotto
        stretchState.handleMouseDragged(100, 140); // move ancora sotto

        verify(shape, atLeastOnce()).stretch(eq(0.0), anyDouble(), eq("DOWN"));
        verify(stretchCommand, atLeastOnce()).accumulate(eq(0.0), anyDouble(), eq("DOWN"));
        verify(controller).addShape(shape, decorator);
    }
