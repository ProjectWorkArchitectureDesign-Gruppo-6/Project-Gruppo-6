...
/********METODO RISERVA*****/
    ...
    public void handleMouseDragged(double x, double y) {
        ...
        if (Math.abs(shape.getDim1()) < 1e-6) shape.setWidth(0.001);
        if (Math.abs(shape.getDim2()) < 1e-6) shape.setHeight(0.001);

        if (shape instanceof Line) {
            Line line = (Line) shape;

            // Calcolo le nuove coordinate finali della linea
            double newX2 = x;
            double newY2 = y;

            // Calcolo le nuove dimensioni della linea
            double newWidth = newX2 - line.getX();  // delta X rispetto a punto iniziale
            double newHeight = newY2 - line.getY(); // delta Y rispetto a punto iniziale

            // Applico le nuove dimensioni
            line.setWidth(newWidth);
            line.setHeight(newHeight);

            // Aggiorno anche x2 e y2 per coerenza, se servono altrove
            line.setX2(newX2);
            line.setY2(newY2);
            shape.resize(factorX, factorY, oldCenterX, oldCenterY);

            // Salta il resto della gestione di ridimensionamento (rotazioni, factorX, factorY, ecc.)
            StateController.getInstance().addShape(shape, shapeView);
            return;
        }
        ...
        if(startX > shape.getX() && startY >= shape.getY()){ // Trascino la maniglia nel vertice in basso a destra
            ...
            System.out.println(shape.getDim2());
        ...
        // gestisci ribaltamento

        boolean flipX = factorX < 0;
        boolean flipY = factorY < 0;

        if (flipX) {
            factorX = -factorX;
            dxRot = -dxRot;
            startX = startX - 2 * dxRot;
        }
        if (flipY) {
            factorY = -factorY;
            dyRot = -dyRot;
            startY = startY - 2 * dyRot;
        }
    ...
    public void handleMouseReleased(double x, double y) {
        ...
        if (!(shape instanceof Line)) {
            ...
            CommandManager.getInstance().executeCommand(currentResizeCommand);
            // Per le Linee, saltiamo il reset del centro perché x, y sono fissi nel resize
            //currentResizeCommand.undo();
        }
        // Ora che logicamente la shape si trova nello stato iniziale, faccio una volta l'execute per riportarla logicamente alla posizione in cui è stata spostata.

       // currentResizeCommand.execute();
        StateController.getInstance().setState(SingleSelectState.getInstance());*/
